...
require 'find'
require File.join(File.dirname(__FILE__), '../findgrep/result')
require 'rubygems'
require 'termcolor'
require 'kconv'
...
#    @patternRegexps = strs2regs(patterns, @option.ignoreCase)
    ...
    @result = FindGrep::Result.new(option.directory)
    open_database
  end
...
EOF
  ...
  def searchPath(path)
    records = @findGrep.documents.select do |record|
      record.path == path
    end
